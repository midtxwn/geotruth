...
func runCompact(ctx context.Context, ops *geotruthops.Ops, args []string) error {
	...
	includeSpatial := fs.Bool("spatial", true, "include SPATIAL source stream compaction")
	includePublic := fs.Bool("public-gt-events", true, "include public GT_EVENTS audit compaction")
	includeTombstones := fs.Bool("removed-tombstones", true, "include removed internal state tombstone compaction")
	if err := fs.Parse(args); err != nil {
		return err
	}
...
func usage() {
	...
	fmt.Fprintf(os.Stderr, "  compact [--execute] [--export-dir DIR] [--spatial=true] [--public-gt-events=true] [--removed-tombstones=true]\n")
}
