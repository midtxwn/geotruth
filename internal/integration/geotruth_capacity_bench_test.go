...
var (
	...
	gtCapacityScenario          = flag.String("scenario", "position", "benchmark scenario: position or geofence-toggle")
...
func readGeoTruthCapacityConfig(b *testing.B) geotruthCapacityConfig {
	...
	switch cfg.scenario {
	case capacityScenarioPosition, capacityScenarioGeofenceToggle:
	default:
...
func runGeoTruthCapacityTrial(b *testing.B, cfg geotruthCapacityConfig) {
	...
	svc, err := embedded.Run(context.Background(), embeddedCfg, benchmarkDependencies(cfg))
	if err != nil {
	...
	b.Logf("capacity summary pass=%v reason=%q scenario=%s objects=%d hz=%d regions=%d sender_workers=%d client_conns=%d gt_pub_workers=%d gt_pub_inflight=%d planned=%d sent=%d accepted=%d errors=%d mean_completion_lag_rate=%.1f/s max_completion_lag=%d end_completion_lag=%d max_driver_lag=%d max_inflight=%d drain_ms=%d commit_p50/p95/p99_ms=%.3f/%.3f/%.3f schedule_p50/p95/p99_ms=%.3f/%.3f/%.3f e2e_p50/p95/p99_ms=%.3f/%.3f/%.3f gt_events_delta_msgs=%d gt_events_delta_bytes=%d",
		pass, reason, cfg.scenario, cfg.objects, cfg.hz, cfg.regions, cfg.senderWorkers, cfg.pool,
...
func runGeoTruthMaxThroughputTrial(b *testing.B, cfg geotruthCapacityConfig) {
	...
	b.Logf("max throughput summary scenario=%s objects=%d regions=%d sender_workers=%d client_conns=%d gt_pub_workers=%d gt_pub_inflight=%d sent=%d accepted=%d errors=%d elapsed_ms=%.3f drain_ms=%.3f completed_updates_per_sec=%.1f accepted_per_trial_sec=%.1f commit_p50/p95/p99_ms=%.3f/%.3f/%.3f gt_events_delta_msgs=%d gt_events_delta_bytes=%d",
		cfg.scenario, cfg.objects, cfg.regions, cfg.senderWorkers, cfg.pool,
		effectiveGTPubWorkers(cfg.gtPubWorkers), effectiveGTPubInflight(cfg.gtPubInflight),
...
func setupCapacityScenario(ctx context.Context, pub natspublish.Publish, query natsquery.Query, scenario string, regions int) error {
	if scenario != capacityScenarioGeofenceToggle {
		return nil
	}
