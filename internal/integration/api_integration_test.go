...
func startServices(tb testing.TB, numFloors int) (*embedded.Services, natspublish.Publish, natsquery.Query) {
	...
	svc, err := embedded.RunLocalStack(context.Background(), cfg, deps)
	if err != nil {
...
func eventIDFromGTEvent(tb testing.TB, data []byte) string {
	...
	if evt.EventID == "" {
		tb.Fatalf("gt event has empty event ID: %s", string(data))
	}
	return evt.EventID
}
...
func TestEndToEnd_DuplicateObjectRegisterRejected(t *testing.T) {
	svc, pub, _ := startServices(t, 1)
	ctx := context.Background()
	dims := domain.ObjectDimensions{Width: 1, Height: 1}

	if _, err := pub.RegisterObject(ctx, "dup-register-obj", dims); err != nil {
		t.Fatalf("first RegisterObject: %v", err)
	}
	stream := gtEventStream(t, ctx, svc.NATSConn())
	before := latestGTEvent(t, ctx, stream, natswatch.GTSubjectObjectRegistered("dup-register-obj"))

	_, err := pub.RegisterObject(ctx, "dup-register-obj", dims)
	if err == nil {
		t.Fatal("duplicate RegisterObject succeeded")
	}
	var remote messages.RemoteError
	if !errors.As(err, &remote) {
		t.Fatalf("duplicate RegisterObject error type = %T, want messages.RemoteError", err)
	}
	if !strings.Contains(remote.Message, "already registered") {
		t.Fatalf("duplicate RegisterObject error = %q, want already registered", remote.Message)
	}

	after := latestGTEvent(t, ctx, stream, natswatch.GTSubjectObjectRegistered("dup-register-obj"))
	if after.Sequence != before.Sequence {
		t.Fatalf("duplicate RegisterObject appended registered event seq %d, want unchanged seq %d", after.Sequence, before.Sequence)
	}
}
...
func TestEndToEnd_AreaLifecycle(t *testing.T) {
	...
	assertEventually(t, func() bool {
		resp, err := query.AllAreas(ctx, nil)
		if err != nil {
			return false
		}
		for _, area := range resp.Regions["0"] {
			if area.ID == "zone-a" {
				return true
			}
		}
	...
	assertEventually(t, func() bool {
		areas, err := query.NearbyAreas(ctx, "0", 5.0, 5.0, 5.0, nil)
		if err != nil {
			return false
		}
		for _, area := range areas {
	...
	assertEventually(t, func() bool {
		areas, err := query.AreasContainingObject(ctx, "obj1", nil)
		if err != nil {
			return false
		}
		for _, area := range areas {
