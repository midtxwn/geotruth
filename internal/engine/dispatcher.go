...
func (d *Dispatcher) onRegister(env *Envelope) {
	if ctl, ok := d.eng.lookupCtl(env.ObjectID); ok {
		...
		ctl.head = env
		d.submitCurrentRegister(ctl, env)
		return
	}
...
func (d *Dispatcher) submitCurrentRegister(ctl *ObjCtl, env *Envelope) {
	state := d.eng.snapshotObjectState(env.ObjectID, ctl, ctl.instanceID, ctl.lastCommitSeq+1)
	msgs, err := gtevents.BuildRegisterCurrentStateCommitMsgs(env.ObjectID, ctl.instanceID, ctl.lastCommitSeq+1, env.ClientOpID, state)
	if err != nil {
		d.rejectHead(ctl, env, err)
		return
	}
...
func (d *Dispatcher) submitBuilt(ctl *ObjCtl, env *Envelope, msgs gtevents.CommitMessages, mutation gtevents.PrevInsideMutation) {
	commitSeq := ctl.lastCommitSeq + 1
	envCommitSeq := commitSeqFromMsg(msgs.StateAfter.CommitSeq, commitSeq)
...
func (d *Dispatcher) dispatchNext(ctl *ObjCtl) {
	...
	switch next.Kind {
	case taskRegister:
		d.submitCurrentRegister(ctl, next)
	case taskUpdate:
