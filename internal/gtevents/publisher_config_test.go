...
func TestPublisherConstructorsNormalizeWorkerCount(t *testing.T) {
	p := NewPublisher(nil, 0)
	if p.workers != defaultPublisherWorkers {
		t.Fatalf("NewPublisher workers = %d, want default %d", p.workers, defaultPublisherWorkers)
	}

	withMonitor := NewPublisherWithMonitor(nil, 3, nil)
	if withMonitor.workers != 3 {
		t.Fatalf("NewPublisherWithMonitor workers = %d, want 3", withMonitor.workers)
	}
}
