...
func TestJetStreamPublisherRequiresVerifierStream(t *testing.T) {
	pub := NewJetStreamPublisher(nil, nil)
	_, err := pub.GetLastMsgForSubject(context.Background(), SubjectPositionUpdated("obj1"))
	if err == nil {
		t.Fatal("expected verifier stream configuration error")
	}
}

func TestPublisherPublishOrVerifyReturnsPublishAck(t *testing.T) {
	msg := msgWithEventID(t, SubjectPositionUpdated("obj1"), "gt:p:obj1:b1i1:2")
	p := &Publisher{}
	ack, err := p.publishOrVerify(context.Background(), fakeMessagePublisher{
		publishAck: &jetstream.PubAck{Stream: natskeys.GTStreamName, Sequence: 7},
	}, msg)
	if err != nil {
		t.Fatalf("publishOrVerify: %v", err)
	}
	if ack == nil || ack.Sequence != 7 || ack.Duplicate {
		t.Fatalf("ack = %+v, want non-duplicate seq 7", ack)
	}
}

func TestPublisherPublishOrVerifyUsesLatestMatchingEventID(t *testing.T) {
	msg := msgWithEventID(t, SubjectPositionUpdated("obj1"), "gt:p:obj1:b1i1:2")
	p := &Publisher{}
	ack, err := p.publishOrVerify(context.Background(), fakeMessagePublisher{
		publishErr: errors.New("publish failed after store"),
...
func TestPublisherPublishOrVerifyReturnsOriginalErrorWhenLatestDiffers(t *testing.T) {
	publishErr := errors.New("publish failed")
	msg := msgWithEventID(t, SubjectPositionUpdated("obj1"), "gt:p:obj1:b1i1:2")
	p := &Publisher{}
	_, err := p.publishOrVerify(context.Background(), fakeMessagePublisher{
		publishErr: publishErr,
		latest: &jetstream.RawStreamMsg{
			Sequence: 42,
			Subject:  msg.Subject,
...
func TestEventIDFromDataRejectsMalformedJSON(t *testing.T) {
	if got := eventIDFromData([]byte(`{"e":`)); got != "" {
		t.Fatalf("eventIDFromData malformed = %q, want empty", got)
	}
}
