...
func RecoverObjectCommits(ctx context.Context, js jetstream.JetStream, bootID string) (map[string]RecoveredCommit, error) {
	...
	for {
		...
		for msg := range msgs.Messages() {
			st, ok, jsonErr := CheckpointStateFromPublicEvent(msg.Data())
			if jsonErr != nil {
				_ = msg.Ack()
				return nil, fmt.Errorf("decode public checkpoint subject %s: %w", msg.Subject(), jsonErr)
			}
			if !ok {
				_ = msg.Ack()
				log.Printf("[gtevents] skipping public object event without checkpoint: subject=%s", msg.Subject())
				continue
			}
...
// CheckpointStateFromPublicEvent rebuilds internal recovery state from a compact
// checkpoint embedded in a public commit event.
func CheckpointStateFromPublicEvent(data []byte) (ObjectStateRecord, bool, error) {
	var event struct {
		...
		InsideAreaIDs []string           `json:"a"`
		Checkpoint    *compactCheckpoint `json:"cp"`
	}
	if err := json.Unmarshal(data, &event); err != nil {
		return ObjectStateRecord{}, false, err
	}
	if event.Checkpoint == nil || event.Checkpoint.Lifecycle == "" {
		return ObjectStateRecord{}, false, nil
	}

	lifecycle, err := expandCompactLifecycle(event.Checkpoint.Lifecycle)
	if err != nil {
		return ObjectStateRecord{}, false, err
	}
	transitions, err := expandCompactTransitions(event.Checkpoint.GeofenceTransitions)
	if err != nil {
		return ObjectStateRecord{}, false, err
	}

	position := event.Position
	if position == nil && event.Checkpoint.Position != nil {
		pos := *event.Checkpoint.Position
		position = &pos
	}

	insideAreaIDs := event.InsideAreaIDs
	if insideAreaIDs == nil && event.Checkpoint.InsideAreaIDs != nil {
		insideAreaIDs = event.Checkpoint.InsideAreaIDs
	}
	if lifecycle == LifecycleRemoved {
		position = nil
		insideAreaIDs = nil
	}
	...
	return ObjectStateRecord{
		...
		Position:            position,
		Dims:                event.Dims,
		InsideAreaIDs:       insideAreaIDs,
		GeofenceTransitions: transitions,
		UpdatedAt:           event.OccurredAt,
	}, true, nil
}

func expandCompactLifecycle(lifecycle string) (Lifecycle, error) {
	switch lifecycle {
	case "a":
	...
	default:
		return LifecycleUnknown, fmt.Errorf("unknown compact checkpoint lifecycle %q", lifecycle)
	}
}

func expandCompactTransitions(transitions []compactCheckpointTransition) ([]GeofenceTransitionRecord, error) {
	if len(transitions) == 0 {
		return nil, nil
	}
	out := make([]GeofenceTransitionRecord, 0, len(transitions))
	for _, tr := range transitions {
		switch tr.Entered {
		case 0:
			out = append(out, GeofenceTransitionRecord{AreaID: tr.AreaID, Entered: false})
		case 1:
			out = append(out, GeofenceTransitionRecord{AreaID: tr.AreaID, Entered: true})
		default:
			return nil, fmt.Errorf("unknown compact checkpoint transition flag %d", tr.Entered)
		}
	}
	return out, nil
}
...
func expectedProjectionMsgs(commit RecoveredCommit) ([]*nats.Msg, error) {
	...
	default:
		return nil, fmt.Errorf("unknown recovered lifecycle %d", commit.State.Lifecycle)
	}
