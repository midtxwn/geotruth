...
func (o *Ops) PlanCompaction(ctx context.Context, opts CompactionOptions) (CompactionPlan, error) {
	...
	if opts.IncludePublicGTEvents {
		for objectID, commits := range byObject {
			...
			for _, rec := range commits {
				...
				if latestRec.Lifecycle == natswatch.LifecycleActive && rec.StreamSeq == latestRec.StreamSeq {
					continue
				}
				if latestRec.Lifecycle == natswatch.LifecycleRemoved && !opts.IncludeRemovedTombstones && rec.StreamSeq == latestRec.StreamSeq {
					continue
				}
	...
	for objectID, recs := range projections {
		...
		for _, rec := range recs {
			if latestRec.Lifecycle == natswatch.LifecycleActive {
...
func (o *Ops) validatePlan(ctx context.Context, plan CompactionPlan) error {
	...
	for _, ref := range plan.DeleteGTEvents {
		if _, ok := allowed[ref.StreamSeq]; !ok {
			return fmt.Errorf("compaction plan tries to delete protected or stale GT_EVENTS seq %d", ref.StreamSeq)
		}
	}
	return nil
}
...
type eventRecord struct {
	...
	CommitSeq           uint64
	Lifecycle           string
	GeofenceTransitions []natswatch.GeofenceTransitionRecord
	IsCommit            bool
	IsProjection        bool
}
...
func parseEventRecord(msg *jetstream.RawStreamMsg) (eventRecord, bool) {
	var meta struct {
		...
		StateAfter *struct {
			...
			GeofenceTransitions []natswatch.GeofenceTransitionRecord `json:"geofence_transitions,omitempty"`
		} `json:"state_after"`
	}
	...
	if meta.StateAfter != nil {
		rec.IsCommit = true
		rec.Lifecycle = meta.StateAfter.Lifecycle
		rec.GeofenceTransitions = meta.StateAfter.GeofenceTransitions
	}
	if strings.Contains(msg.Subject, ".geofence.") {
		rec.IsProjection = true
	}
	return rec, rec.ObjectID != ""
}
