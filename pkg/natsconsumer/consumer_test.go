...
func TestSubjectMatches(t *testing.T) {
	tests := []struct {
		...
		{"gt.events.v1.>", "gt.events.v1.geotruth.booted", true},
		...
		{"gt.events.v1.geotruth.booted", "gt.events.v1.geotruth.booted", true},
		{"gt.events.v1.geotruth.booted", "gt.events.v1.geotruth.other", false},
...
func TestTrieInsertAndMatch(t *testing.T) {
	...
	tri.insert("gt.events.v1.geotruth.booted", e3)
	...
	matches = tri.match("gt.events.v1.geotruth.booted")
	if len(matches) != 2 {
		t.Fatalf("expected 2 matches, got %d", len(matches))
	}
	ids = make(map[uint64]bool)
...
func TestTrieWildcardGtMidMatch(t *testing.T) {
	...
	tests := []struct {
		...
		{"gt.events.v1.geotruth.booted", 0},
	}
	for _, tt := range tests {
		matches := tri.match(tt.subject)
		if len(matches) != tt.expected {
...
func TestTrieRemoveNonWildcard(t *testing.T) {
	...
	e1 := subEntry{id: 1}
	tri.insert("gt.events.v1.geotruth.booted", e1)
	...
	removed := tri.remove("gt.events.v1.geotruth.booted", 1)
	if removed.id != 1 {
		t.Errorf("expected removed id 1, got %d", removed.id)
	}
