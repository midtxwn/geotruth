...
func DetectorBooted(ctx context.Context, cons *natsconsumer.Consumer) (
	events <-chan DetectorBootedEvent, unsubscribe func(), err error) {

	msgCh, unsub, err := cons.Subscribe(ctx, GTDetectorBooted)
	if err != nil {
		return nil, nil, fmt.Errorf("subscribe for detector booted: %w", err)
	}

	ch := make(chan DetectorBootedEvent, 256)
	go func() {
		defer close(ch)
		for {
			select {
