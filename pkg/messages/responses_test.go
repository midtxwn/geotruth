...
func TestErr_OnOKDataResp(t *testing.T) {
	...
	err := Err(wrapped)
	if err != nil {
		t.Errorf("messages.Err should return nil for OKDataResp with source_seq data, got: %v", err)
	}
}

func TestData_ExtractsSourceSeq(t *testing.T) {
	wrapped := OKDataResp(struct {
		SourceSeq uint64 `json:"source_seq"`
	}{SourceSeq: 42})
	...
	if parsed.SourceSeq != 42 {
		t.Errorf("expected source_seq 42, got %d", parsed.SourceSeq)
	}
}
