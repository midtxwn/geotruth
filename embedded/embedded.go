...
type Config struct {
	Ingester ingester.Config  `mapstructure:"ingester"`
...
// connection factories internally; use RunIngester or RunGeoTruth to run a
// service with external caller-provided dependencies.
...
// RunIngester starts only the Ingester service with caller-provided
// dependencies, including the NATS connector.
func RunIngester(ctx context.Context, cfg ingester.Config, deps ingester.Dependencies) (Service, error) {
	ing, err := appingester.Run(ctx, cfg, deps)
	if err != nil {
		return nil, err
	}
	select {
...
func Run(ctx context.Context, cfg Config, deps Dependencies) (*Services, error) {
	...
	ing, err := RunIngester(stackCtx, cfg.Ingester, ingester.Dependencies{
	...
	if err != nil {
		cancel()
		natsSvc.Shutdown()
		return nil, fmt.Errorf("ingester run: %w", err)
	}
